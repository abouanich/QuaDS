...
def clamod(result, vchi2):
    ...
    * create the cla/mod statistics for each vchi2, variables and modalities
    ...
    for g, h, i, j in zip(lv_cm, ln_cm, index_cm, l_cm):
        clamod = clamod.append(
            {vchi2: g, "variables": h, "modalities": i, "cla/mod": j}, ignore_index=True
        )
        clamod = clamod.fillna(0)
    for i in range(len(result)):
        for j in range(len(clamod)):
            if (
                str(result[vchi2][i]) == str(clamod[vchi2][j])
...
def modcla(result, vchi2):
    ...
    * create the mod/cla statistics for each vchi2, variables and modalities
    ...
    for g, h, i, j in zip(lv_mc, ln_mc, i_modcla, l_mc):
        modcla = modcla.append(
            {vchi2: g, "variables": h, "modalities": i, "mod/cla": j}, ignore_index=True
        )
    ...
    for i in range(len(result)):
        ...
        for j in range(len(modcla)):
            if (
                str(result[vchi2][i]) == str(modcla[vchi2][j])
                and str(result["variables"][i]) == str(modcla["variables"][j])
                and str(result["modalities"][i]) == str(modcla["modalities"][j])
            ):
...
def globa(result):
    ...
    * create the global statistics for each vchi2, variables and modalities
    ...
    for h, i, j in zip(variables_2, index_g, l_g):
        ...
        glo = glo.append(
            {"variables": h, "modalities": i, "global": j}, ignore_index=True
        )

    for i in range(len(result)):
        # add the global column from glo to global column from result
        for j in range(len(glo)):
            if str(result["variables"][i]) == str(glo["variables"][j]) and str(
                result["modalities"][i]
            ) == str(glo["modalities"][j]):
...
def pvalue(result, vchi2):
    ...
    * calculate the p-value
    ...
    for h, i, j in zip(variables_2, index_nj, l_nj):
        ...
        table_nj = table_nj.append(
            {"variables": h, "modalities": i, "nj": j}, ignore_index=True
        )
    ...
    for i, j in zip(i_nk, l_nk):
        table_nk = table_nk.append({vchi2: i, "nk": j}, ignore_index=True)
    for c in new_df:
        for v in dictio:
    ...
    for g, h, i, j in zip(lv_nkj, ln_nkj, index_nkj, l_nkj):
        table_nkj = table_nkj.append(
            {vchi2: g, "variables": h, "modalities": i, "nkj": j}, ignore_index=True
        )
    ...
    for i in range(len(table)):
        for j in range(len(table_nkj)):
            if (
                str(table[vchi2][i]) == str(table_nkj[vchi2][j])
                and str(table["variables"][i]) == str(table_nkj["variables"][j])
                and str(table["modalities"][i]) == str(table_nkj["modalities"][j])
            ):
                table["nkj"][i] = table_nkj["nkj"][j]

        # attribute the nj number from table_nj to table
        for j in range(len(table_nj)):
            if str(table["variables"][i]) == str(table_nj["variables"][j]) and str(
                table["modalities"][i]
            ) == str(table_nj["modalities"][j]):
    ...
    for i in range(len(result)):
        for j in range(len(table)):
            if (
                str(result[vchi2][i]) == str(table[vchi2][j])
                and str(result["variables"][i]) == str(table["variables"][j])
                and str(result["modalities"][i]) == str(table["modalities"][j])
            ):
                nj = table["nj"][j]
                nk = table["nk"][j]
                nkj = table["nkj"][j]
    ...
    for i in range(len(result)):
        for j in range(len(table)):
            ...
            if result["p-value"][i] < v_p_value:
                result["v-test"][i] = round(
                    (1 - 2 * val) * norm.ppf(result["p-value"][i] / 2), 7
                )
            else:
...
def variable_weight(result):
    ...
    * calcule the weight of a variable counting the number of modality
    ...
    for i in range(len(result)):
        ...
        if (
            result["signification"][i] == "overrepresented"
            or result["signification"][i] == "underrepresented"
        ):
...
def sdquanti(df, var, variable_cat):
    ...
    * Make the anova on each quantitative variable with variable_cat
    ...
    anova = pd.DataFrame(
        {"variable": var, "Eta2": eta, "p-value": table["PR(>F)"][0]}, index=[0]
    )
    return anova


def quanti_analysis(anova, df, var, variable_cat, pv_anova, pv_vtest):
    """
    Actions performed:
    * Make the v-test and final statistics
    ...
    for el in range(len(df_na)):
        ...
        sous_ov = np.append(
            sous_ov, np.array([[float(df_na[var][el]), float(x)]]), axis=0
        )
    som_ov = 0
    for i in sous_ov:
    ...
    for i, j in zip(dictionary_1, dictionary):
        ...
        for el in range(len(data)):
            ...
            sous_cat = np.append(
                sous_cat, np.array([[float(data[var][el]), float(xq)]]), axis=0
            )
        som_cat = 0
        for k in sous_cat:
        ...
        sd_cat.append(et_category)
        ov_sd.append(et_overall)
    if anova["p-value"][0] < pv_anova:
        ...
        for element in range(len(sign)):
            if sign[element] == "Not significant":
                v[element] = "Not significant"
        quantitative = pd.DataFrame(
            {
                variable_cat: variable,
                "Mean in category": mc,
                "Overall mean": om,
                "Sd in category": sd_cat,
                "Overall sd": ov_sd,
                "p-value": pv,
                "v-test": v,
                "signification": sign,
...
if __name__ == "__main__":
    ...
    df2 = pd.read_csv(
        r"kmeans_semantic_cluster_coordinates6.csv", sep=";", encoding="latin-1"
    )
    ...
    dataframe_quanti = dataframe_quanti.rename(
        columns={"Number of flowers by volume": "Number_of_flowers_by_volume"}
    )
    ...
    # vtest = vtest(quali_a,'cluster',0.05)
    # w = variable_weight(quali_a)

    # out :
    # sdquali=sdquali.rename_axis('file : 20220520_florhige_synthese_english, code : 20220520_sdqua')
    # sdquali.to_csv('semantic_x2_cluster6_kmeans.csv')
    # vtest=vtest.rename_axis('file : 20220520_florhige_synthese_english, code : 20220520_sdqua')
    # vtest.to_csv("semantic_sdquali_cluster6_kmeans.csv")
    # w=w.rename_axis('file : 20220520_florhige_synthese_english, code : 20220520_sdqua')
    # w.to_csv('semantic_weight_cluster6_kmeans.csv')

    # make the quantitative analysis for each quantitative variable
    sdquanti = sdquanti(dataframe_quanti, "Number_of_flowers_by_volume", "cluster")
    quanti_a = quanti_analysis(
        sdquanti, dataframe_quanti, "Number_of_flowers_by_volume", "cluster", 0.05, 0.05
    )

    # out :
    sdquanti = sdquanti.rename_axis(
        "file : 20220520_florhige_synthese_english, code : 20220520_sdqua"
    )
    sdquanti.to_csv("semantic_anova_cluster6_kmeans.csv")
    quanti_a = quanti_a.rename_axis(
        "file : 20220520_florhige_synthese_english, code : 20220520_sdqua"
    )
    quanti_a.to_csv("semantic_sdquanti_cluster6_kmeans.csv")
